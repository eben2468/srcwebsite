<?php
/**
 * Simple Chat Setup - Safe Version
 * This script creates the chat system database tables without depending on users table structure
 */
...
echo "<!DOCTYPE html>";
echo "<html><head><title>Chat System Setup - Safe Version</title>";
...
echo "<h1>🚀 Chat System Database Setup (Safe Version)</h1>";
...
echo "<h3>Step 4: Setting Up Agent Status (Safe Mode)</h3>";

// Check if users table exists
$checkUsers = $conn->query("SHOW TABLES LIKE 'users'");
if ($checkUsers && $checkUsers->num_rows > 0) {
    echo "<p class='info'>✅ Users table found</p>";
    
    // Get all user IDs without assuming column names
    $userIdsResult = $conn->query("SELECT user_id FROM users LIMIT 10");
    
    if ($userIdsResult && $userIdsResult->num_rows > 0) {
        $agentCount = 0;
        while ($user = $userIdsResult->fetch_assoc()) {
            $userId = $user['user_id'];
            ...
            if ($conn->query($sql) === TRUE) {
                echo "<p class='success'>✅ Setup agent status for User ID: $userId</p>";
                $agentCount++;
            } else {
                echo "<p class='error'>❌ Error setting up agent for User ID $userId: " . $conn->error . "</p>";
            }
        }
        echo "<p class='info'>Total agents configured: $agentCount</p>";
    } else {
        echo "<p class='info'>ℹ️ No users found in the users table.</p>";
    }
} else {
    echo "<p class='info'>ℹ️ Users table not found. Creating default agent status entries.</p>";
    
    // Create default agent status entries for user IDs 1-5
    for ($i = 1; $i <= 5; $i++) {
        $sql = "INSERT IGNORE INTO chat_agent_status (agent_id, status, max_concurrent_chats, auto_assign) 
                VALUES ($i, 'offline', 5, 1)";
        
        if ($conn->query($sql) === TRUE) {
            echo "<p class='success'>✅ Created default agent status for User ID: $i</p>";
        }
    }
    echo "<p class='info'>Created default agent status entries. These will be used when users are created.</p>";
}
...
echo "<ol>";
echo "<li><a href='pages_php/support/test_api.php' target='_blank'>Test the API</a></li>";
...
echo "<ul>";
echo "<li>Agent status has been set up for existing users</li>";
echo "<li>All admin, super_admin, and member users can act as chat agents</li>";
echo "<li>Users need to set their status to 'online' in chat management to receive chats</li>";
echo "<li>The system will automatically assign chats to available agents</li>";
echo "</ul>";
