...
$basePath = '';
if (basename(dirname($_SERVER['PHP_SELF'])) === 'pages_php') {
    $basePath = '../';
}

// Include required files if not already included
if (!function_exists('fetchAll')) {
    require_once $basePath . 'db_config.php';
}
...
if (empty($galleryImages)) {
    $galleryDir = $basePath . 'uploads/gallery/';
    ...
    <div class="content-card-body p-0">
        ...
        <div id="<?php echo $carouselId; ?>" class="gallery-carousel">
            <div class="gallery-carousel-inner">
                ...
                $imageGroups = array_chunk($galleryImages, 3);
                foreach ($imageGroups as $groupIndex => $imageGroup): 
                ?>
                <div class="gallery-carousel-item <?php echo ($groupIndex === 0) ? 'active' : ''; ?>" data-index="<?php echo $groupIndex; ?>">
                    <div class="gallery-carousel-row">
                        <?php foreach ($imageGroup as $image): ?>
                        <div class="gallery-carousel-col">
                                ...
                                <img src="<?php 
                                    ...
                                    if (isset($image['file_name'])) {
                                        ...
                                        // Check if it's a full path
                                            ...
                                            $imagePath = $basePath . $image['file_name'];
                                        }
                                        // Check if file exists in uploads/gallery
                                        else if (file_exists($basePath . 'uploads/gallery/' . $image['file_name'])) {
                                            $imagePath = $basePath . 'uploads/gallery/' . $image['file_name'];
                                        }
                        ...
                        <?php 
                        // If we don't have 3 images in this group, add empty placeholders
                        $emptySlots = 3 - count($imageGroup);
                        for ($i = 0; $i < $emptySlots; $i++): 
                        ?>
                        <div class="gallery-carousel-col empty"></div>
                        <?php endfor; ?>
                    </div>
                </div>
                <?php endforeach; ?>
            ...
            <div class="gallery-carousel-indicators">
                <?php foreach ($imageGroups as $groupIndex => $imageGroup): ?>
                <button type="button" class="<?php echo ($groupIndex === 0) ? 'active' : ''; ?>" 
                        data-index="<?php echo $groupIndex; ?>" 
                        aria-label="Slide <?php echo $groupIndex + 1; ?>"></button>
                <?php endforeach; ?>
            </div>
        </div>
        <?php endif; ?>
...
.gallery-carousel-col:hover {
    transform: translateY(-5px);
}

.gallery-carousel-col.empty {
    visibility: hidden;
}
...
.gallery-image-container:hover .gallery-carousel-image {
    filter: brightness(1);
...
@media (max-width: 768px) {
    ...
    .gallery-carousel-col:not(:first-child) {
        display: none;
    }
