...
// Include auth functions for shouldUseAdminInterface
...
function hasFeaturePermission($feature, $adminOnly = false) {
    ...
    if ($adminOnly && !shouldUseAdminInterface()) {
        return false;
    }
    
    // Always allow admins and super admins to access features even when disabled
    if (shouldUseAdminInterface()) {
        return true;
    }
    
    // Check if the feature is enabled for non-admin users
